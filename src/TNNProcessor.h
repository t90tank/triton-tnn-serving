...
public:
  TNNProcessor(const std::string &name,
                const int device_id,
                const std::shared_ptr<TNN_NS::InputShapesMap> shapeMap) : name_(name), device_id_(device_id), shape_map_(shapeMap) {}
  ...
  bool SetInputMat(const void *input_buffer, const std::string &input_name); 
...
private:
  ...
  std::shared_ptr<TNN_NS::InputShapesMap> shape_map_; 
